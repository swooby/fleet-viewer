...
namespace RTEditor
{
    ...
    public static class GameObjectExtensions
    {
        ...
        public static void PlaceHierarchyOnPlane(this GameObject root, Vector3 ptOnPlane, Vector3 planeNormal, int alignAxisIndex)
        {
            ...
            Plane plane = new Plane(planeNormal, ptOnPlane);
            if (alignAxisIndex >= 0 && alignAxisIndex < 3) root.AlignAxis(planeNormal, alignAxisIndex);
            ...
            if (pivotPtIndex < 0) pivotPtIndex = plane.GetIndexOfClosestPointInFrontOrOnPlane(oobbPts);
            if (pivotPtIndex >= 0)
            {
